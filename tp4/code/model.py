...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        hs, _ = self.rnn(self.embedding(input))
        return hs
...
class seq2seqAtt(nn.Module):
    ...
    def forward(self, target_h, source_hs):
        ...
        scores = self.ff_score(self.tanh(self.ff_concat(concat_output))) # should be of shape (seq,batch,1)
...
class Decoder(nn.Module):
    ...
    def forward(self, input, source_context, h):
        ...
        _, h = self.rnn(self.embedding(input))

        concat = torch.cat([source_context, h[-1].unsqueeze(0)], 2)
        h_tilde = self.tanh(self.ff_concat(concat))
...
class seq2seqModel(nn.Module):
    ...
    def forward(self, input, max_size, is_prod, plot_att=False):
        ...
        target_input = (
            torch.LongTensor([self.sos_token]).repeat(current_batch_size).unsqueeze(0).to(self.device)
        )  # init (1,batch)
        ...
        while True:
            ...
            if self.do_att:
                source_context, scores = self.att_mech(target_h, source_hs)  # (1,batch,feat)
                att_scores.append(scores.detach().cpu().squeeze().numpy())
            else:
            ...
            # fill the gap #
            # use the decoder
            prediction, target_h = self.decoder(target_input, source_context, target_h)  # def forward(self, input, source_context, h):
            ...
            # print(eos_counter, current_batch_size)
        ...
        if is_prod:
            ...
            if plot_att:
                return to_return, np.array(att_scores)
    ...
    def targetInts_to_nl(self, target_ints):
        ...
        return [
            ...
            if elt == self.sos_token
            else self.vocab_t_inv[elt]
            for elt in target_ints
        ]

    def predict(self, source_nl, plot_att=False):
        source_ints = self.sourceNl_to_ints(source_nl)

        if not plot_att:
            logits = self.forward(
                source_ints, self.max_size, True, plot_att=plot_att
                )  # (seq) -> (<=max_size,vocab)
            target_ints = logits.argmax(-1).squeeze()  # (<=max_size,1) -> (<=max_size)
            target_nl = self.targetInts_to_nl(target_ints.tolist())
            return " ".join(target_nl)
        else:
            logits, scores = self.forward(
                source_ints, self.max_size, True, plot_att=plot_att
                )  # (seq) -> (<=max_size,vocab)
            target_ints = logits.argmax(-1).squeeze()  # (<=max_size,1) -> (<=max_size)
            target_nl = self.targetInts_to_nl(target_ints.tolist())

            return " ".join(target_nl), scores
