...
if not is_prod:
    ...
    def plot_alignement(sentence, trad, scores):
        from nltk import word_tokenize

        x_ticks_labels = word_tokenize(sentence, "english")
        y_ticks_labels = word_tokenize(trad, "french")
        # print(y_ticks_labels)

        plt.xticks(range(len(x_ticks_labels)), x_ticks_labels, rotation=90)
        plt.yticks(range(len(y_ticks_labels)), y_ticks_labels)

        plt.imshow(scores)
        plt.savefig(f'{sentence}.png')
        plt.show()


    for elt in to_test:
        trad, scores = model.predict(elt, plot_att=True)
        print("= = = = = \n", "%s -> %s" % (elt, trad))

        plot_alignement(elt, trad, scores)


    for elt in ["I did not mean to hurt you", "She is so mean",]:
        trad, scores = model.predict(elt, plot_att=True)
        print("= = = = = \n", "%s -> %s" % (elt, trad))
