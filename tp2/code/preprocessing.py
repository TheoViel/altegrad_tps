...
path_read = "for_moodle/data/"
path_write = "for_moodle/output/"
...
print('''Index of "don't" :''', word_to_index["don't"])
...
for i, rev in enumerate(cleaned_reviews):
    ...
    for t in rev:
        try:
            sublist.append(word_to_index[t])
        except KeyError:
            sublist.append(0)
    reviews_ints.append(sublist)

    if not i % (len(cleaned_reviews) // 5):
        print("Text :", rev)
        print("Indexed text :", sublist)
        print('')
