...
y = torch.LongTensor(np.argmax(class_labels, axis=1)).cuda()
...
model = GNN(features.shape[1], n_hidden_1, n_hidden_2, n_class, dropout_rate).cuda()
...
labels = np.argmax(class_labels[idx_test.cpu(), :], axis=1)
